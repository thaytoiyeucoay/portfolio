...
import { ExternalLink, Paperclip, Upload, Search, Send, Sparkles, FileText, Settings, Plus, Trash2, RefreshCw, Link as LinkIcon, BookOpen, Info, Quote, Copy, Check } from "lucide-react";
  ...
  return (
    <div className="relative h-7 w-7 shrink-0 rounded-full bg-emerald-500/15 ring-1 ring-emerald-500/30 grid place-items-center overflow-hidden">
      <svg viewBox="0 0 40 40" className="h-6 w-6">
        ...
        {/* mouth */}
        <rect x="14" y="24" width="12" height="3" rx="1.5" className={talking ? "origin-center animate-mouth-open fill-current" : "fill-current"} />
      </svg>
      <style jsx>{`
        @keyframes mouthOpen {
          0% { transform: scaleY(0.6); opacity: 0.9; }
          50% { transform: scaleY(1.6); opacity: 1; }
          100% { transform: scaleY(0.6); opacity: 0.9; }
        }
        .animate-mouth-open { animation: mouthOpen 0.9s ease-in-out infinite; }
      `}</style>
    </div>
  );
}
  ...
  return (
    <div className="container mx-auto max-w-6xl px-4 py-10">
      <div className="mb-6 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          <h1 className="text-2xl font-semibold">Chatbot RAG</h1>
            ...
            <CardContent className="p-0">
              <div className="h-[540px] w-full overflow-auto p-4">
                <div className="mx-auto max-w-3xl space-y-6">
                  {messages.map((m, idx) => (
                    <div key={m.id} className="rounded-lg border border-white/10 bg-background/60 p-4">
                      <div className="mb-3 flex items-center justify-between">
                        <div className="flex items-center gap-2 text-xs text-muted-foreground">
                          {m.role === "assistant" ? (
                            ...
                            <div className="flex items-center gap-2">
                              <div className="grid h-6 w-6 place-items-center rounded-full bg-blue-500/20 text-[10px]">U</div>
              ...
              <div className="border-t border-white/10 p-3">
                <div className="mx-auto flex max-w-3xl items-end gap-2">
                  ...
                  />
                  <Button className="shrink-0" onClick={onAsk} disabled={disabled}>
                    {sending ? "Đang gửi..." : (
                      <span className="inline-flex items-center gap-2"><Send className="size-4" /> Gửi</span>
                    )}
                  </Button>
                </div>
              </div>
